...
def tobit_model_censorc(label):
    ...
    df = df.drop(columns=['Charging Time (mins)', 'Parking Time (mins)','# Food', '# Shop & Service',
       ...
       '# Travel & Transport', '# College & University', '# Event',
       '# Residence', 'Plugs_raw', 'Number of NEMA 5-20R_raw','Number of NEMA 5-20R', 'Label_0', 'Label_1', 'Label_2', 'Label_3',
       'Label_4', 'Label_5', 'Label_6', 'Label_7', 'Fee (USD)'])
    # df = df[df["Label"]==label]
...
def tobit_model_plugs(label):
    ...
    testdata["Energy (kWh)_y"] = scaler_eng.fit_transform(np.array(testdata["Energy (kWh)_y"]).reshape(-1,1))
    
    #testdata["Fee (USD)_y"] = scaler_fee.fit_transform(np.array(testdata["Fee (USD)_y"]).reshape(-1,1))

    testdata = l.buildLaggedFeatures(testdata,["Energy (kWh)_y"], lag=8)

    # testdata = testdata.apply(censor, axis=1)
    

    t = Tobit(testdata, 'Censored', list(testdata.drop(columns=["Energy (kWh)_y","Censored"]).columns), 'Energy (kWh)_y')

    regressor = Ridge(fit_intercept=False).fit(testdata.drop(columns=["Energy (kWh)_y", "Censored"]), testdata["Energy (kWh)_y"])
    pred = regressor.predict(testdata.drop(columns=["Energy (kWh)_y","Censored"]))
    ...
    pred_tobit = t.predict(np.array(testdata.drop(columns=["Energy (kWh)_y","Censored"])),beta)
    ...
    predictions["Tobit_pred"] = scaler_eng.inverse_transform(pred_tobit)
    predictions["Ridge_pred"] = scaler_eng.inverse_transform(pred)
    predictions["Censored_energy"] = testdata["Energy (kWh)_y"]

    return predictions

#for label in range(1):
#   tobit_model_plugs(label).to_csv(f"data\createdDat\Censorship_scheem\Tobit_plug_censor_label{label}.csv")
